...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.digital_graphic?.length > 0) {
      const create_digital_graphic = body.digital_graphic;
      body.digital_graphic = {
        create: create_digital_graphic,
      };
    } else {
      delete body.digital_graphic;
    }
